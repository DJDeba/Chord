...
import { Guitar} from '../assets/img/index'
...
const Header = () => {
  ...
  return (
    <header className='flex items-center w-full p-4 md:py-2 md:px-6'>
        <NavLink to={"/"}>
        <img src={Guitar} alt="Logo" className='w-16' />
        </NavLink>
            ...
            {isMenu && (
              ...
              className='absolute z-10 top-12 p-3 right-0 w-275 gap-2 bg-card shadow-lg rounded-lg backdrop-blur-sm flex flex-col'>
                  <NavLink to={'/userProfile'}>
                    <p className='text-base text-textColor hover:text-headingColor font-semibold duration-150 transition-all ease-in-out'>Profile</p>
                  </NavLink>
                  <p className='text-base text-textColor hover:text-headingColor font-semibold duration-150 transition-all ease-in-out'>My Favourites</p>
                  <hr />
                 {
                  user?.user?.role === "admin" && (
                  <>
                    <NavLink to={'/dashboard/home'}>
                      <p className='text-base text-textColor hover:text-headingColor font-semibold duration-150 transition-all ease-in-out'>Dashboard</p>
                    </NavLink>
                    <hr />
                  </>
                  )
                 }
                  <p className='text-base text-textColor hover:text-headingColor font-semibold duration-150 transition-all ease-in-out' onClick={logOut}>Sign Out</p>
              </motion.div>
            )}
        </div>
    </header>
